# Support API Academy: Guia Interno

> **Este arquivo é só para uso interno.** Ele fica listado no `.gitignore` de propósito e não deve ir para o repositório público. Guarde uma cópia local (fora do Git) para consulta.

Trilha técnica de APIs e Integrações do Suporte C2S, versão 5.2.
...
├── data.js           → onde ficam os ENDPOINTS, CASOS, VIDEO_GROUPS e CHANGELOG
...
├── README.md         → versão pública e curta, vai para o repositório
├── GUIA-INTERNO.md   → este arquivo, uso interno, listado no .gitignore
└── .gitignore        → impede o GUIA-INTERNO.md de ir para o repositório público
```

## Sobre deixar o repositório público

Ao publicar este repositório como público no GitHub, tudo que estiver commitado fica visível para qualquer pessoa, incluindo o conteúdo completo de `data.js`. Isso inclui os **links dos vídeos do Loom** dentro de `VIDEO_GROUPS`.

**Importante:** deixar o repositório do GitHub privado **não** resolve isso sozinho. O site, pra funcionar, entrega o `data.js` inteiro pro navegador de qualquer visitante da página publicada, então o link continua acessível a quem inspecionar o código do site, independente da visibilidade do repositório no GitHub.

**Decisão do time (atualizada):** os vídeos devem ser privados de verdade, não só "quem tem o link". A forma de garantir isso sem custo nenhum e sem mexer no código do site é configurar a privacidade de cada vídeo direto no Loom:

1. Abra o vídeo no Loom, botão Share.
2. Troque o link de Public para "Only people in your workspace" (ou restrito ao domínio de email da empresa).
3. Repita isso em todo vídeo já publicado (hoje: `autenticacao-get`, `leads-get`, `leads-post`).
4. Se o plano do Loom permitir, defina essa opção como padrão em Workspace Settings → Security → Access & viewing, pra não esquecer nos próximos vídeos.

Com isso, mesmo que o link apareça no `data.js` público, quem não tiver conta Loom vinculada ao domínio da empresa não consegue assistir. Assistir no Loom é sempre gratuito, só quem grava precisa de plano pago.
